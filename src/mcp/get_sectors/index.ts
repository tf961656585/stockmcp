...
export const getSectorsTool: McpTool = {
  spec: {
    name: 'get_sectors',
    description: '获取板块数据（注意：AllTick API不直接提供板块分类数据）',
    inputSchema: { 
      type: 'object', 
      properties: {
        codes: { 
  ...
  async handle(args: any, { dataFetcher }: ToolContext) {
    ...
    try {
      // AllTick 不提供板块分类数据，但可以获取相关个股的数据
      ...
      return { 
        content: [{ 
          type: 'text', 
          text: JSON.stringify({
            说明: 'AllTick API不直接提供板块分类数据，以下为相关个股的实时数据',
            个股实时数据: result,
            请求参数: {
              查询代码: codes
            },
